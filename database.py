...
def _init_schema(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS product_settings (
            product_id  INTEGER PRIMARY KEY,   -- id from the irMarket catalogue
            enabled     INTEGER NOT NULL DEFAULT 1,
            markup      REAL,                   -- NULL = use the global markup
            category_id INTEGER,
            updated_at  REAL NOT NULL,
...
async def get_or_create_category(name: str) -> int:
    ...
    async with _lock:
        ...
        if row:
            return row["id"]
        cur = conn.execute(
            "INSERT INTO categories (name, created_at) VALUES (?, ?)", (name, time.time())
        )
        conn.commit()
        return cur.lastrowid


async def list_categories() -> list[sqlite3.Row]:
    async with _lock:
        conn = _require_conn()
...
async def delete_category(category_id: int) -> None:
    async with _lock:
        conn = _require_conn()
        conn.execute(
            "UPDATE product_settings SET category_id = NULL WHERE category_id = ?",
            (category_id,),
        )
...
# ------------------------------------------------------- product settings --
async def get_all_product_settings() -> dict[int, sqlite3.Row]:
    async with _lock:
        conn = _require_conn()
        rows = conn.execute("SELECT * FROM product_settings").fetchall()
        return {r["product_id"]: r for r in rows}


async def _ensure_product_row(conn: sqlite3.Connection, product_id: int) -> None:
    conn.execute(
        "INSERT OR IGNORE INTO product_settings (product_id, enabled, updated_at) "
        "VALUES (?, 1, ?)",
        (product_id, time.time()),
    )


async def set_product_enabled(product_id: int, enabled: bool) -> None:
    async with _lock:
        conn = _require_conn()
        await _ensure_product_row(conn, product_id)
        conn.execute(
            "UPDATE product_settings SET enabled = ?, updated_at = ? WHERE product_id = ?",
            (1 if enabled else 0, time.time(), product_id),
        )
        conn.commit()


async def set_product_markup(product_id: int, markup: Optional[float]) -> None:
    """markup=None clears the override, falling back to the global markup."""
    async with _lock:
        conn = _require_conn()
        await _ensure_product_row(conn, product_id)
        conn.execute(
            "UPDATE product_settings SET markup = ?, updated_at = ? WHERE product_id = ?",
            (markup, time.time(), product_id),
        )
        conn.commit()


async def set_product_category(product_id: int, category_id: Optional[int]) -> None:
    async with _lock:
        conn = _require_conn()
        await _ensure_product_row(conn, product_id)
        conn.execute(
            "UPDATE product_settings SET category_id = ?, updated_at = ? WHERE product_id = ?",
            (category_id, time.time(), product_id),
        )
        conn.commit()
