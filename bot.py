...
async def show_products(query, page: int):
    ...
    rows = []
    for p in chunk:
        price = helpers.retail_price(p["price_usd"], current_markup)
...
async def show_product_detail(query, product_id: int, qty: int = 1):
    ...
    unit_price = helpers.retail_price(product["price_usd"], current_markup)
...
async def show_admin_menu(query):
    ...
    rows = [
        ...
        [InlineKeyboardButton(f"📈 درصد سود ({percent_now}%)", callback_data="admin:markup")],
